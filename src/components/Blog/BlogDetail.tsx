...
const renderMarkdown = (markdown: string) => {
  ...
  const html = markdown
    ...
    // Code blocks
    .replace(/```(.*?)```/gs, (match, p1) => 
      `<pre class="bg-gray-100 p-4 rounded-lg my-5 overflow-x-auto"><code class="text-sm font-mono text-gray-800">${p1.trim()}</code></pre>`
    );
...
const BlogDetail = () => {
    ...
    if (id) {
      ...
      if (foundPost) {
        ...
        const related = posts
          .filter(p => 
            p.id !== foundPost.id && 
            (p.author_id === foundPost.author_id || 
  ...
  const handleAuthorClick = (authorId: string) => {
    navigate('/blog', { state: { filterAuthor: authorId } });
  };
  
  const handleTagClick = (tagId: string) => {
    navigate('/blog', { state: { filterTag: tagId } });
  };
            ...
            <div className="flex items-start bg-gray-50 p-5 rounded-lg">
              {author?.avatar && (
                <img 
                  src={author.avatar} 
                  alt={author.name} 
                  className="w-16 h-16 rounded-full mr-5 border-2 border-white shadow"
                />
              )}
              <div>
                <button 
        ...
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {relatedPosts.map(relatedPost => {
              ...
              return (
                <div key={relatedPost.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-all transform hover:-translate-y-1">
                  <Link to={`/blog/${relatedPost.id}`} className="block">
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center text-sm">
                        <img 
                          src={relatedPostAuthor?.avatar} 
                          alt={relatedPostAuthor?.name} 
